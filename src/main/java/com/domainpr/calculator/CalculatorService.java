...
//    }
    ...
    // domain 계층에서 유효성 검증을 해보자
        Calculator calculator = new Calculator(stringArray);

        // 다른 구분자가 쓰이는지 검증
        ArrayList<String> fixedDistinguisherAndNumbers = calculator.ifAnotherDistinguisher(stringArray);

        // 입력값에 숫자가 아닌 값 혹은 음수가 있는지 검증
        return calculator.ifNotNumberOrNegative(fixedDistinguisherAndNumbers);
